            ...
            confirmationResult.confirm(formData.otp).then((result) => {
                ...
                dispatch(callApi(options))
                    .then((res) => {
                        ...
                        if (mode === 'SIGNIN') {
                            if (res.payload?.user) {
                                dispatch(storeCustomerData({ ...res.payload.user, agent: res.payload.agent }));
                                localStorage.setItem("customer", JSON.stringify({ ...res.payload.user, agent: res.payload.agent }));
            ...
            <div className='menu_popup' onMouseEnter={() => setVisited(true)}>
                {popupStage === 0 ? (
                    ...
                    <div className='btns_group'>
                        ...
                        {/* Customer */}
                        <button className='popup_btn' onClick={() => routeEntry('CUSTOMER')}>Sign Up As Customer</button>
                        <button className='popup_btn' onClick={() => routeEntry('AGENT')}>Sign Up As Broker</button>
                    </div>
                )}
            </div>
        );
